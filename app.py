...
def workshop_page():
    if request.method == 'POST':
        ...
        workshop_data = {
            ...
            'slot': request.form.get('slot'),
            'extension_boxes': request.form.get('extension_boxes'),
            'no_of_mic':request.form.get('no_of_mic'),
            'mic_reason':request.form.get('mic_reason')
        }
...
def generate_pdf_ws(filepath, event_data):
    ...
    pdf.line(start_x, top_line_y, start_x, bottom_line_y+92)  # Left vertical line
    pdf.line(width - start_x, top_line_y, width - start_x, bottom_line_y+92)  # Right vertical line
    ...
    pdf.drawString(start_x + 10, start_y, f"No of Extension box: {event_data.get('extension_boxes')}")
    start_y-=20
    pdf.drawString(start_x + 10, start_y, f"Reason : {event_data.get('extension_reason')}")
    start_y -= 30  # Space before the horizontal line
    pdf.line(start_x, start_y, width - start_x, start_y)  # Horizontal line
    start_y -= 25  # Space before the "NUMBER REQUIRED" section
    pdf.drawString(start_x + 10, start_y, f"No of mic: {event_data.get('no_of_mic')}")
    start_y-=20
    pdf.drawString(start_x + 10, start_y, f"Reason : {event_data.get('mic_reason')}")
    start_y -= 30  # Space before the horizontal line
    pdf.line(start_x, start_y, width - start_x, start_y)  # Horizontal line
    start_y -= 40
...
def generate_pdf_content_pp(filepath,event_data):
    ...
    pdf.circle(margin + 43, height -66, 5,  fill=1 if event_data.get("day")=="day_1" else 0)

    pdf.drawString(margin + 80, height - 70, "Day 2:")
    pdf.circle(margin + 123, height - 66, 5, fill=1 if event_data.get("day")=="day_2" else 0 )

    pdf.drawString(margin + 170, height - 70, "Day 3:")
    pdf.circle(margin + 213, height - 66, 5,fill=1 if event_data.get("day")=="day_3" else 0 )

    pdf.drawString(margin + 300, height - 70, "Both Days:")
    pdf.circle(margin + 367, height - 66, 5, fill=1 if event_data.get("day")=="both_days" else 0 )
...
@app.route('/presentation_search')
def presentation_search():
    return render_template('presentation_search.html')
...
def search_event():
    ...
    elif search_id.startswith("PRPN"):
        # Fetch event data from the database
